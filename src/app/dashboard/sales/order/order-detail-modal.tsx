...
export function OrderDetailModal({
    ...
    useEffect(() => {
        if (isOpen && order?.id) {
            ...
            fetchOrderDetail(order.id).then((res) => {
                if (isMounted) {
    ...
    useEffect(() => {
        if (isOpen && detailedOrder) {
            ...
            if (detailedOrder.order_date) {
                setOrderDate(format(new Date(detailedOrder.order_date), 'yyyy-MM-dd'))
            } else if (detailedOrder.created_at) {
                setOrderDate(format(new Date(detailedOrder.created_at), 'yyyy-MM-dd'))
            } else {
                setOrderDate('')
            }

            if (detailedOrder.due_date) {
                setExpectedShipDate(format(new Date(detailedOrder.due_date), 'yyyy-MM-dd'))
            } else {
                setExpectedShipDate('')
            }

            const firstItem = detailedOrder.order_items?.[0]
            if (firstItem) {
    ...
    const handleSave = async () => {
        ...
        setIsLoading(true)
        try {
            const firstItemId = detailedOrder?.order_items?.[0]?.id
    ...
    const handleConfirm = async () => {
        ...
        if (bomItems.length === 0) {
            toast.error('최소 1개의 BOM(부품) 내역이 있어야 이관할 수 있습니다.')
            return
        }
            ...
            <SheetContent className="w-full sm:max-w-[1000px] overflow-y-auto bg-slate-950 border-l-border/50 p-0 hide-scrollbar [&>button]:hidden">
                ...
                {isFetchingDetail ? (
                    <div className="flex flex-col items-center justify-center p-20 opacity-50 space-y-4">
                        ...
                        <p className="text-sm font-medium">BOM 및 특이사항 상세 데이터를 불러오는 중입니다...</p>
                    </div>
                ) : (
